...
# Flag to enable parallel processing during mesh creation
PARALLEL = True

# Flag to run the script in test mode (selecting a subset of regions)
TEST = False

# Function to download atlas data
def download_atlas_files(download_dir_path: Path, atlas_file_url: str) -> Path:
    ...
    # Check if the atlas data has already been downloaded
    if atlas_files_dir.exists():
        print("Not downloading atlas since it was downloaded already")
...
# Function to parse structure data and create a hierarchy
...
# Function to create a hierarchical structure of anatomical structures
...
# Function to create 3D meshes for anatomical structures
def create_meshes(download_dir_path, structures, annotated_volume, root_id):
    ...
    # Determine regions to create meshes for
...
# Function to create a dictionary mapping structure IDs to their mesh files
...
def create_atlas(working_dir):
    ...
    # Wrap up, compress, and remove files
    print("Finalizing atlas")
